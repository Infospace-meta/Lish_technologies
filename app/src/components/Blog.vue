<template>
  <div class="container mx-auto animate-fadeIn">
    <div class="text-blue-950 font-bold text-4xl lg:text-6xl text-center mt-16">
      Blog
    </div>
    <div
      class="text-blue-950 text-lg lg:text-xl text-center mt-6 lg:mt-12 font-semibold"
    >
      Stay up to date with the latest trends and insights in technology with our
      informative blog. <br />From industry news to expert tips, we've got you
      covered.
    </div>
    <div
      class="flex flex-col lg:flex-row justify-center lg:justify-between gap-20 items-center mt-8 lg:mt-12"
    >
      <div class="text-center lg:text-left">
        <div class="w-full lg:w-96 h-0 border-b border-gray-800 mt-12"></div>
        <div class="text-blue-950 font-semibold">
          T E C H <br /><br /><span
            class="text-blue-950 font-bold text-xl lg:text-2xl"
            >Why Cloud Computing is <br />Essential for your Business <br />in
            Year 2024 </span
          ><br /><br /><br /><span class="text-2xl">01.03</span>
        </div>
        <div class="w-full lg:w-96 h-0 border-b border-gray-800 mt-12"></div>
      </div>

      <div class="text-center transition hover:scale-95">
        <div
          class="max-w-sm p-6 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700 mt-12"
        >
          <span class="text-white">T I P S</span><br /><br /><span
            class="font-bold text-2xl text-white"
            >How to Protect Business <br />
            from Cybersecurity <br />Threats <br /></span
          ><span class="text-white">12 April 2024 | TeamWaveNet</span
          ><br /><br />
          <button
            class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 border border-blue-700 rounded animate-bounce"
          >
            READ MORE
          </button>
        </div>
      </div>

      <div class="text-center lg:text-left">
        <div class="w-full lg:w-96 h-0 border-b border-gray-800 mt-12"></div>
        <div class="text-blue-950 font-semibold">
          R E V I E W <br /><br /><span
            class="text-blue-950 font-bold text-xl lg:text-2xl"
            >5 Ways Software Developers <br />Can Boost Your Business
            <br />Quicker Than Ever </span
          ><br /><br /><br /><span class="text-2xl">20.05</span>
        </div>
        <div class="w-full lg:w-96 h-0 border-b border-gray-800 mt-12"></div>
      </div>
    </div>
    <div class="text-center mt-8 lg:mt-12">
      <button
        class="mx-auto border border-blue-950 text-blue-950 font-bold py-2 px-4 rounded-md flex items-center justify-center animate-bounce"
      >
        READ MORE ARTICLES
        <svg
          xmlns="http://www.w3.org/2000/svg"
          width="16"
          height="16"
          fill="currentColor"
          class="bi bi-arrow-right ml-2"
          viewBox="0 0 16 16"
        >
          <path
            fill-rule="evenodd"
            d="M1 8a.5.5 0 0 1 .5-.5h11.793l-3.147-3.146a.5.5 0 0 1 .708-.708l4 4a.5.5 0 0 1 0 .708l-4 4a.5.5 0 0 1-.708-.708L13.293 8.5H1.5A.5.5 0 0 1 1 8"
          />
        </svg>
      </button>
    </div>
    <br /><br />
  </div>
</template>
